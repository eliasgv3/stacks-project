...
The axioms TR1-TR4 are self-dual.
\begin{lemma}
	\label{lemma-opposite-triangulated-category}
	Let $(\mathcal{D},[\ ],\mathcal{T})$
	be the data in Definition \ref{definition-triangulated-category}.
%	Denote $\Theta:\mathcal{D}\to\mathcal{D}^\text{opp}$
%	to the assignment that sends an object to itself
%	and a morphism $f:X\to Y$ to the opposite morphism $f^\text{opp}:Y\to X$.
	For each $n\in\mathbb{Z}$, define $[n]'= [-n]^\mathrm{op}:\mathcal{D}^\mathrm{op}\to\mathcal{D}^\mathrm{op}$;
	for each triangle $(X,Y,Z,f,g,h)$ of $\mathcal{D}$, define
	$(X,Y,Z,f,g,h)^\mathrm{op}
	=(Z,Y,X,g^\mathrm{op},f^\mathrm{op},(-h[-1])^\mathrm{op})$
	and define $\mathcal{T}^\mathrm{op}
	=\{\Delta^\mathrm{op}\mid\Delta\in\mathcal{T}\}$.
	Then $(\mathcal{D},[\ ],\mathcal{T})$ is a (pre-)triangulated category
	if and only if $(\mathcal{D},[\ ],\mathcal{T})^\mathrm{op}
	:=(\mathcal{D}^\mathrm{op},[\ ]',\mathcal{T}^\mathrm{op})$ is.
\end{lemma}

\begin{proof}
	It suffices to show that $(\mathcal{D},[\ ],\mathcal{T})$ (pre-)triangulated
	implies $(\mathcal{D},[\ ],\mathcal{T})^\text{op}$ (pre-)triangulated,
	for $((\mathcal{D},[\ ],\mathcal{T})^\text{op})^\text{op}
	=(\mathcal{D},[\ ],\mathcal{T})$.
	
	\medskip\noindent
	Suppose that $(\mathcal{D},[\ ],\mathcal{T})$ is a pre-triangulated category.
	Then $(\mathcal{D},[\ ],\mathcal{T})^\text{op}$ satisfies TR2,
	since if we consider the opposite sequence of \eqref{equation-rotate} in
	$(\mathcal{D}^\text{op},[\ ]')$,
	then any four consecutive terms form a distinguished triangle of $\mathcal{T}^\text{op}$.
	Using again the opposite sequence of \eqref{equation-rotate},
	we deduce TR1 for $(\mathcal{D},[\ ],\mathcal{T})^\text{op}$.
	Lastly, TR3 for $(\mathcal{D},[\ ],\mathcal{T})^\text{op}$ follows from TR3 for
	$(\mathcal{D},[\ ],\mathcal{T})$ plus TR2 for
	$(\mathcal{D},[\ ],\mathcal{T})^\text{op}$.
	
	\medskip\noindent
	We omit the verification that $(\mathcal{D},[\ ],\mathcal{T})$
	triangulated implies $(\mathcal{D},[\ ],\mathcal{T})^\text{opp}$ triangulated.
\end{proof}
...
Note that the translation $[n]$ in a pre-triangulated category is an exact automorphism.
If $(F,\xi):\mathcal{D}\to\mathcal{D}'$
and $(G,\theta):\mathcal{D}'\to\mathcal{D}'')$ are exact functors of pre-triangulated categories,
the composition is defined as $(G,\theta)\circ(F,\xi)=(G\circ F,\theta_F\circ {}_G\xi)$
(notation from Categories, Formal properties, \ref{section-formal-cat-cat}),
i.e., $(\theta_F\circ {}_G\xi)_X=\theta_{F(X)}\circ G(\xi_X)$.
...
\begin{definition}
	Let $\mathcal{D}$, $\mathcal{D}'$ be triangulated categories. A contravariant triangulated functor from $\mathcal{D}$ to $\mathcal{D}'$ is a triangulated functor $F:\mathcal{D}^\mathrm{op}\to\mathcal{D}'$, where $\mathcal{D}^\mathrm{op}$ is defined in Lemma \ref{lemma-opposite-triangulated-category}.
\end{definition}
Equivalently, a contravariant triangulated functor from $\mathcal{D}$ to $\mathcal{D}'$ is a contravariant functor $F$ from $\mathcal{D}$ to $\mathcal{D}'$ equipped with an isomorphism of functors $\xi:F\circ[-1]^\mathrm{op}\to [1]\circ F$ (where $[-1]^\mathrm{op}$ is the opposite functor of some quasi-inverse of $[1]:\mathcal{D}\to\mathcal{D}$) such that for every distinguished triangle $A\to B\to C\to A[1]$ in $\mathcal{D}$ we have that $F(C)[1]\xleftarrow{-\xi_C\circ F(h[-1])}F(A)\leftarrow F(B)\leftarrow F(C)$ is a distinguished triangle in $\mathcal{D}'$.
...
MS6 follows from TR3, the long exact cohomology sequence
(\ref{equation-long-exact-cohomology-sequence})
and Homology, Lemma \ref{lemma-five-lemma}.
By
Lemma \ref{lemma-localization-conditions}
we conclude that MS2 holds. To finish the proof we have to show that
MS3 holds. To do this let $f, g : X \to Y$ be morphisms of $\mathcal{D}$,
and let $t : Z \to X$ be an element of $S$ such that $f \circ t = g \circ t$.
As $\mathcal{D}$ is additive this simply means that $a \circ t = 0$ with
$a = f - g$. Choose a distinguished triangle $(Z, X, Q, t, d, h)$ using
...
i.e., $j \in S$. Finally, $j \circ a = j \circ i \circ d = 0$ as
$j \circ i = 0$. Thus $j \circ f = j \circ g$ and we see that LMS3 holds.
The proof of RMS3 is dual.
\end{proof}

Ultimately, given a pre-triangulated category $\mathcal{D}$ and a multiplicative system
$S$ compatible with the triangulated structure,
we want to endow $S^{-1}\mathcal{D}$ with the structure of a
pre-triangulated category that makes the localization functor
$Q:\mathcal{D}\to S^{-1}\mathcal{D}$ exact. Imposing this condition
completely determines the class of distinguished triangles in
$S^{-1}\mathcal{D}$: a triangle in $S^{-1}\mathcal{D}$ wil be
distinguished if and only if it is isomorphic to the image by $Q$ of
a distinguished triangle in $\mathcal{D}$.
This is what the next lemma shows.

\begin{lemma}
	\label{lemma-localization-dist-triangle}
	Let $\mathcal{D}$ be a pre-triangulated category and $S$ be a
	left multiplicative system in $\mathcal{D}$.
	Suppose $S^{-1}\mathcal{D}$ is equipped with a pre-triangulated
	structure for which the localization functor
	$F:\mathcal{D}\to S^{-1}\mathcal{D}$, along with a natural isomorphism
	$\xi:F\circ[1]_{\mathcal{D}}\to[1]_{S^{-1}\mathcal{D}}\circ F$,
	is exact.
	Then any distinguished triangle in $S^{-1}\mathcal{D}$ is
	isomorphic to some $(Q(X),Q(Y),Q(Z),Q(f),Q(g),\xi_X\circ Q(h))$,
	where $(X,Y,Z,f,g,h)$ is a distinguished triangle in $\mathcal{D}$.
\end{lemma}

\begin{proof}
	Let $(Q(X),Q(Y),Q(Z),a,b,c)$ be
	a distinguished triangle in $S^{-1}\mathcal{D}$.
	Write $a = Q(s)^{-1} \circ Q(f)$ for
	some $s : Y \to Y'$ in $S$ and $f : X \to Y'$. Extend $f$ to a distinguished triangle
	$(X,Y',W,f,g,h)$ in $\mathcal{D}$.
	Then $(Q(X),Q(Y'),Q(W),Q(f),Q(g),\xi_X\circ Q(h))$ is a distinguished triangle
	in $S^{-1}\mathcal{D}$, for $Q$ is assumed to be exact.
	We can use TR3 in $S^{-1}\mathcal{D}$ to find $d:Q(Z)\to Q(W)$ such that
	$(\text{id}_{Q(X)},Q(s),d)$ is a morphism of distinguished triangles
	$(Q(X),Q(Y),Q(Z),a,b,c)
	\to(Q(X),(Y'),Q(W),Q(f),Q(g),\xi_X\circ Q(h))$.
	By Lemma \ref{lemma-third-isomorphism-triangle}, this is a triangle isomorphism.
\end{proof}
...
$S^{-1}\mathcal{D}$ such that $Q(f)[1]=Q(f[1])$,
for all morphisms $f$ of $\mathcal{D}$, and for which the localization functor
$Q : \mathcal{D} \to S^{-1}\mathcal{D}$ is exact, where $\xi_X=\text{id}_{Q(X)[1]}=\text{id}_{Q(X[1])}$.
Moreover, if $\mathcal{D}$ is a triangulated category, so is
$S^{-1}\mathcal{D}$.
\end{proposition}

\begin{proof}
For the uniqueness, first note that since $Q$ is exact implies $Q$ is additive (Lemma \ref{lemma-exact-functor-additive}), the preadditive structure on
$S^{-1}\mathcal{D}$ is fully determined by that of $\mathcal{D}$,
by Homology, Lemma \ref{homology-lemma-localization-preadditive}.
The class of distinguished triangles in $S^{-1}\mathcal{D}$ is also
fully determined by that of $\mathcal{D}$,
by Lemma \ref{lemma-localization-dist-triangle}.
On the other hand, thanks to MS5 the translation $[1]:S^{-1}\mathcal{D}\to S^{-1}\mathcal{D}$ exists and is uniquely determined by Categories, Lemma \ref{categories-lemma-properties-left-localization}.
%On the other hand, the action on objects of the translation in
%$S^{-1}\mathcal{D}$ is uniquely determined.
%But so is the action on morphisms,
%since funtoriality implies
%$(s^{-1}f)[1]
%=(Q(s)^{-1}\circ Q(f))[1]
%=(Q(s)[1])^{-1}\circ(Q(f)[1])
%=Q(s[1])^{-1}\circ Q(f[1])
%=(s[1])^{-1}(f[1])$.
%By induction, one obtains $(s^{-1}f)[n]=(s[n])^{-1}(f[n])$.
	
\medskip\noindent
It is left to show existence.
...
If $f:X\to Y'$ is a morphism in $\mathcal{D}$
and $s:Y'\to Y$ is a morphism of $S$,
it is clear that we may define
$(s^{-1}f)[n] = (s[n])^{-1}(f[n])$ since
...
Let $\mathcal{D}$ be a pre-triangulated category. Let $S$ be left multiplicative system on $\mathcal{D}$.
Suppose that $S^{-1}\mathcal{D}$ is equipped with the structure
of a pre-triangulated category for which the localization functor
$(Q,\xi) : \mathcal{D} \to S^{-1}\mathcal{D}$ is exact.
\begin{enumerate}
...
\item Let $(F,\zeta) : \mathcal{D} \to \mathcal{D}'$ be an exact functor into
a pre-triangulated category $\mathcal{D}'$ such that $F(s)$ is an isomorphism
for all $s \in S$. Then, there exists a unique exact functor
$(F',\zeta') : S^{-1}\mathcal{D} \to \mathcal{D}'$ such that $(F,\zeta) =(F',\zeta') \circ (Q,\xi)$.
\end{enumerate}
\end{lemma}

\begin{proof}
Part (1) follows from Lemma \ref{lemma-localization-dist-triangle}.
We prove part (2).
Existence and uniqueness of $F'$ is provided by Categories, Lemma \ref{categories-lemma-properties-left-localization},
so we have to show existence and uniqueness of $\zeta'$.
We first tackle uniqueness.
If $(F,\zeta)
=(F',\zeta')\circ(Q,\xi)
=(F'\circ Q,\zeta'_Q\circ{}_{F'}\xi)$,
we have $\zeta_X=\zeta'_{Q(X)}\circ F'(\xi_X)$,
so $\zeta'_{Q(X)}=\zeta_X\circ F'(\xi_X)^{-1}$
is uniquely determined by $(F,\zeta)$ and $(Q,\xi)$.

\medskip\noindent
For the existence, we \emph{define}
$\zeta'_{Q(X)}=\zeta_X\circ F'(\xi_X)^{-1}:
F'(Q(X)[1])\to F'(Q(X))[1]$.
To show naturality of $\zeta'$, note that since every morphism in $S^{-1}\mathcal{D}$
is of the form $Q(s)^{-1}\circ Q(f)$, where $s\in S$,
it suffices to show naturality for morphisms of the form $Q(f)$.
Indeed, given a morphism $f:X\to Y$ in $\mathcal{D}$,
naturality of $\zeta$ and $\xi$ implies commutativity of the diagram:
\[
\xymatrix{
	F'(Q(X)[1]) \ar@{->}[r]^(.4){F'(\xi_X)^{-1}} \ar@{->}[d]_{{F'(Q(f)[1])}} & F'(Q(X[1]))=F(X[1]) \ar@{->}[r]^(.7){\zeta_X} \ar@{->}[d]|-{{F'(Q(f[1]))=F(f[1])}} & F(X)[1] \ar@{->}[d]^{{F(f)[1]}} \\
	F'(Q(Y)[1]) \ar@{->}[r]_(.4){F'(\xi_Y)^{-1}} & F'(Q(Y[1]))=F(Y[1]) \ar@{->}[r]_(.7){\zeta_Y} & F(Y)[1]
}
\]
Finally, given a distinguished triangle
$\Delta=(Q(X),Q(Y),Q(Z),a,b,c)$ in $S^{-1}\mathcal{D}$,
we must show that
$F'(\Delta)=(F(X),F(Y),F(Z),F'(a),F'(b),\zeta'_{Q(X)}\circ F'(c))$
is distinguished.
By Lemma \ref{lemma-localization-dist-triangle}, it suffices to show it for
$\Delta=(Q(X),Q(Y),Q(Z),Q(f),Q(g),\xi_X\circ Q(h))$,
where $(X,Y,Z,f,g,h)$ is a distinguished triangle in $\mathcal{D}$.
Then $\zeta'_{Q(X)}\circ F'(\xi_X\circ Q(h))
=\zeta'_{Q(X)}\circ F'(\xi_X)\circ F(h)
=\zeta_X\circ F(h)$ and
$F'(\Delta)=(F(X),F(Y),F(Z),F(a),F(b),\zeta_X\circ F(h))$
is distinguished, for $(F,\zeta)$ is exact.
\end{proof}

\begin{lemma}
	\label{lemma-localization-unique-pre-triangulated-structure}
	Let $\mathcal{D}$ be a pre-triangulated category, and let $S$ be
	a multiplicative system in $\mathcal{D}$ compatible with the pre-triangulated structure.
	There exists a pre-triangulated structure on $S^{-1}\mathcal{D}$, unique up to unique exact isomorphism,
	that makes the localization functor $Q:\mathcal{D}\to S^{-1}\mathcal{D}$ be exact.
	More precisely, suppose
	$([\ ]_{S^{-1}\mathcal{D}},\mathcal{T}_{S^{-1}\mathcal{D}})$
	and $([\ ]'_{S^{-1}\mathcal{D}},\mathcal{T}'_{S^{-1}\mathcal{D}})$
	pre-triangulated structures on $S^{-1}\mathcal{D}$ for which
	there are natural transformations $\xi$ and $\xi'$ that turn
	the localization functor into an exact functor of pre-triangulated categories,
	\begin{align*}
		(Q,\xi):
		(\mathcal{D},[\ ]_{\mathcal{D}},\mathcal{T}_{\mathcal{D}})
		\to(S^{-1}\mathcal{D},[\ ]_{S^{-1}\mathcal{D}},\mathcal{T}_{S^{-1}\mathcal{D}}),\\
		(Q,\xi'):
		(\mathcal{D},[\ ]_{\mathcal{D}},\mathcal{T}_{\mathcal{D}})
		\to(S^{-1}\mathcal{D},[\ ]'_{S^{-1}\mathcal{D}},\mathcal{T}'_{S^{-1}\mathcal{D}}).
	\end{align*}
	Then there is a unique isomorphism of functors
	$\zeta:[1]_{S^{-1}\mathcal{D}}\cong[1]'_{S^{-1}\mathcal{D}}$ such that $\xi'=\zeta_Q\circ\xi$.
	Hence, $(\text{id}_{S^{-1}\mathcal{D}},\zeta):
	(S^{-1}\mathcal{D},[\ ]_{S^{-1}\mathcal{D}},\mathcal{T}_{S^{-1}\mathcal{D}})
	\to(S^{-1}\mathcal{D},[\ ]'_{S^{-1}\mathcal{D}},\mathcal{T}'_{S^{-1}\mathcal{D}})$
	is an isomorphism of pre-triangulated categories.
\end{lemma}

\begin{proof}
	To verify that $(\text{id}_{S^{-1}\mathcal{D}},\zeta)$ preserves distinguished triangles,
	by Lemma \ref{lemma-localization-dist-triangle} it suffices to verify it for the image by $(Q,\xi)$
	of a distinguished triangle in $\mathcal{D}$.
	Uniqueness then follows from Lemma \ref{lemma-universal-property-localization}.
	On the other hand, existence is proven in Lemma \ref{proposition-construct-localization}.
\end{proof}
...
Homology, Lemma \ref{homology-lemma-long-exact-sequence-cochain}
the map $B^\bullet \to A^\bullet$ is a quasi-isomorphism.
