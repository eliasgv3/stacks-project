...
$f : K^\bullet \to (L')^\bullet$, $s : L^\bullet \to (L')^\bullet$
defines a morphism in $\text{Qis}^{+}(\mathcal{A})^{-1}K^{+}(\mathcal{A})$
which is zero in $D(\mathcal{A})$. By Homology,
Lemma \ref{lemma-morphism-kernel-localization}, there exists a
...
\begin{proof}
We only prove (1), for the proof of (2) is dual.
The first claim follows from the fact that precomposition
by $s$ induces a cofinal functor $Y/S\to X/S$ and
by Categories, Lemma \ref{categories-lemma-cofinal-essentially-constant}.

It is left to see that $RF(s)$ is an isomorphism. The diagram of functors

$$
\xymatrix{
	Y/S \ar@{->}[rr] \ar@{->}[rd] &  & X/S \ar@{->}[ld] \\
	& \mathcal{D}' & 
}
$$
commutes, where the horizontal map is precomposition
by $s$ and the vertical maps are the ones from
Definition \ref{definition-right-derived-functor-defined}(1).
Thus, we get a natural map of colimits $g:RF(Y)\to RF(X)$, that is an
isomorphism by Categories, Lemma \ref{categories-lemma-cofinal}.
We are going to show that $g^{-1}=RF(s)$.
Suppose that we have a commutative diagram
$$
\xymatrix{
	X \ar@{->}[d]_{s} \ar@{->}[r]^{t} & X' \ar@{->}[d]^{f'} \\
	Y \ar@{->}[r]^{t} & Y'}
$$
with $t,t'\in S$.
Then, by the way the natural morphism $g$ is defined, the diagram
$$
\xymatrix{
	F(X) \ar@{->}[d] \ar@{->}[r] & F(X') \ar@{->}[r] \ar@{->}[d] & RF(X) \\
	F(Y) \ar@{->}[r] & F(Y') \ar@{->}[r] & RF(Y) \ar@{->}[u]_{g}}
$$
commutes.
By the uniqueness in Lemma \ref{lemma-derived-functor},
it follows then $g^{-1}=RF(s)$.
\end{proof}
...
\begin{proof}
To prove (1), apply Categories,
Lemmas \ref{categories-lemma-cofinal-essentially-constant} and \ref{categories-lemma-cofinal}
to the commutative diagram of functors
$$
	\xymatrix{
	X/S \ar@{->}[d] \ar@{->}[r] & X[n]/S \ar@{->}[d] \\
	\mathcal{D}' \ar@{->}[r]^{{[n]}} & \mathcal{D}'
	}
$$
where the top map is induced by $[n]$ ($S$ satisfies MS5) and the vertical maps
are described in
Definition \ref{definition-right-derived-functor-defined}(1).
(Use also that the colimit of the composite
$X/S\to\mathcal{D}'\xrightarrow{[n]}\mathcal{D}'$
is the same as applying $[n]$ to the colimit of $X/S\to\mathcal{D}'$,
for $[n]$ is an isomorphism.)
The proof of (2) is dual.
\end{proof}

The following is a technical lemma that is used
in the proof that $RF$ is an exact functor of triangulated categories,
Lemmas \ref{lemma-2-out-of-3-defined} and \ref{proposition-derived-functor}.

\begin{lemma}
	\label{lemma-derived-functor-second-definition}
	Assumptions and notation as in Situation \ref{situation-derived-functor}.
	Suppose $RF$ is defined at $X,Y$. There are morphisms $s:S\to X'$,
	$s':Y\to Y'$ in $S$, $f:X'\to Y'$ in $\mathcal{D}$ and
	$\alpha:RF(X)\to F(X')$, $\beta: RF(Y)\to F(Y')$ as in
	Categories, Definition \ref{categories-definition-essentially-constant-diagram}
	such that the diagrams
	\[
	\xymatrix{
		X \ar@{->}[r]^{s} \ar@{->}[d]_{f} & X' \ar@{->}[d]^{f'} & 
		& RF(X) \ar@{->}[r]^{\alpha} \ar@{->}[d]_{RF(f)}
		& F(X') \ar@{->}[d]^{F(f')} \\
		Y \ar@{->}[r]^{s'} & Y' &  & RF(Y) \ar@{->}[r]^{\beta} & F(Y')
	}
	\]
	commute.
	Moreover, $RF(f)$ is the unique morphism $RF(X)\to RF(Y)$
	for which such $s,s',f',\alpha,\beta$
	exist making these diagrams commute.
\end{lemma}

\begin{proof}
	Pick objects $X\to X''$, $X\to X'''$ of $X/S$
	for which there are maps $RF(X)\to F(X'')$, $RF(X)\to F( X''')$
	satisfying Categories, Definition \ref{categories-definition-essentially-constant-diagram}.
	In particular, $F( X''')\to RF(X)$ is epic.
	There are maps $X''\to X'$, $ X'''\to X'$ in $X/S$,
	such that the map $F( X''')\to F(X')$ equals
	$F( X''')\to RF(X)\to F(X'')\to F(X')$.
	Note that the morphism $ X'''\to X'$ is in $S$,
	for $S$ is saturated (Categories, Lemma \ref{categories-lemma-what-gets-inverted}).
	By MS2, there is a commutative diagram
	\[
	\xymatrix{
		X \ar@{->}[d]_{f} \ar@{->}[r] &  X''' \ar@{->}[d] \\
		Y \ar@{->}[r] &  Y'''
	}
	\]
	with $Y\to  Y'''$ in $S$.
	Pick an object $Y\to\tilde{Y}''$ of $Y/S$ for which there
	is a morphism $RF(Y)\to F(\tilde{Y}'')$ satisfying
	Categories, Definition \ref{categories-definition-essentially-constant-diagram}.
	There are morphisms $ Y'''\to Y''$,
	$\tilde{Y}''\to Y''$ in $Y/S$ such that $F( Y''')\to F(Y'')$
	equals the composite
	$F( Y''')\to RF(Y)\to F(\tilde{Y}'')\to F(Y'')$.
	We have that $ Y'''\to Y''$ is in $S$ for $S$ is saturated.
	By MS2, there is a morphism $Y''\to Y'$ in $S$ and
	$f':X'\to Y'$ such that the following diagram commutes:
	\[
	\xymatrix{
		 X''' \ar@{->}[rr] \ar@{->}[d] &  & X' \ar@{->}[d]^{f'} \\
		 Y''' \ar@{->}[r] & Y'' \ar@{->}[r] & Y'
	}
	\]
	...
	\xymatrix{
		F( X''') \ar@{->}[r] \ar@{->}[d]
		& RF(X) \ar@{->}[r] \ar@{->}[d]^{RF(f)}
		& F(X'') \ar@{->}[rr] &  & F(X') \ar@{->}[d]^{F(f')} \\
		F( Y''') \ar@{->}[r] & RF(Y) \ar@{->}[r]
		& F(\tilde{Y}'') \ar@{->}[r] & F(Y'') \ar@{->}[r] & F(Y')
	}
	\]
	We claim that the right rectangle commutes
	(the one between $RF(X)$ and $F(Y')$). To prove this,
	it suffices to see commutativity after precomposition
	with the epimorphism $F(X''')\to RF(X)$.
	Commutativity of the right rectangle then follows
	from commutativity of the left square
	(Lemma \ref{lemma-derived-functor}) plus commutativity
	of the second-to-last diagram. We are done after naming
	the composites $s:X\to X'''\to X'$, $s':Y\to Y'''\to Y''\to Y'$,
	$\alpha:RF(X)\to F(X'')\to F(X')$,
	$\beta:RF(Y)\to F(\tilde{Y}'')\to F(Y'')\to F(Y')$.
	(Categories, Remark \ref{categories-remark-essentially-constant-down-the-diagram}.)
	
	\medskip\noindent
	We now show uniqueness of $RF(f)$.
	Suppose there is $a:RF(X)\to RF(Y)$ such
	that we have commutative diagrams
	\[
			\xymatrix{
				X \ar@{->}[r]^{s} \ar@{->}[d]_{f} & X' \ar@{->}[d]^{f'}
				&  & RF(X) \ar@{->}[r]^{\alpha} \ar@{->}[d]_{a}
				& F(X') \ar@{->}[d]^{F(f')} \\
				Y \ar@{->}[r]^{s'} & Y' &  & RF(Y) \ar@{->}[r]^{\beta} & F(Y')
			}
	\]
	where $s,s'\in S$ and $\alpha,\beta$ are as in
	Categories, Definition \ref{categories-definition-essentially-constant-diagram}.
	Let
	\begin{equation*}
		\xymatrix{
	...
	\end{equation*}
	be a commutative diagram, with the horizontal maps in $S$.
	By the uniqueness in Lemma \ref{lemma-derived-functor},
	proving $a=RF(f)$ amounts to show commutativity of
	\begin{equation*}
		\xymatrix{
			F(X'') \ar@{->}[r] \ar@{->}[d] & RF(X) \ar@{->}[d]^{a} \\
			F(Y'') \ar@{->}[r] & RF(Y)
		}
	\end{equation*}
	We are going to show commutativity of this last diagram
	after postcomposition with a certain monomorphism.
	Invoking the defining property of $\alpha,\beta$,
	we get maps $X''\to X'''$, $X'\to X'''$ in $X/S$,
	and $Y''\to Y_1'''$, $Y'\to Y_1'''$ in $Y/S$
	such that the maps $F(X'')\to F(X''')$ and
	$F(Y'')\to F(Y_1''')$ respectively equal the composites
	$F(X'')\to RF(X)\to F(X')\to F(X''')$ and
	$F(Y'')\to RF(Y)\to F(Y')\to F(Y_1''')$.
	By MS2, there is a map $Y'\to Y_2'''$ in $S$
	and a map $X'''\to Y_2'''$ such that the square
	\[
	\xymatrix{
		X' \ar@{->}[r] \ar@{->}[d] & X''' \ar@{->}[d] \\
		Y' \ar@{->}[r] & Y_2'''
	...
	\]
	commutes. Since $Y/S$ is filtered, for $i=1,2$
	we get a map $Y_i'''\to Y'''$ in $Y/S$ such that
	$g_1=g_2$, where $g_i=(Y'\to Y_i'''\to Y''')$.
	Hence, the diagram
	\[
	\xymatrix{
		F(X'') \ar@{->}[r] & RF(X) \ar[d]^a \ar@{->}[r]
		& F(X') \ar@{->}[r] \ar@{->}[d] & F(X''') \ar@{->}[d] \\
		F(Y'') \ar@{->}[r] & RF(Y) \ar@{->}[r] & F(Y') \ar@{->}[r] & F(Y''')
	}
	\]
	commutes. At this stage, note that the composite
	$RF(Y)\to F(Y')\to F(Y''')$ is a monomorphism
	(Categories, Remark \ref{categories-remark-essentially-constant-down-the-diagram}).
	However, it is not clear that if we add $F(X'')\to F(Y'')$,
	then the outer square will commute.
	For this reason, we do the following:
	By MS2, there is a morphism $Y'''\to \tilde{Y}'''$ in $S$
	and a morphism $X'''\to\tilde{Y}'''$
	such that the following diagram commutes:
	\begin{equation}
		\label{diagram-uniqueness-second-definition-derived-functor}
		\xymatrix{
			X'' \ar@{->}[rr] \ar@{->}[d] &  & X''' \ar@{->}[d] \\
			Y'' \ar@{->}[r] & Y''' \ar@{->}[r] & \tilde{Y}''
		}
	\end{equation}
	Here, the triangle
	\[
	...
	\]
	may not commute, so we will use MS3 to make it commute
	after postcomposition with some map in $S$.
	...
	\xymatrix{
		X'' \ar@{->}[ddd] \ar@/^/@{->}[rrrd] &  &  &  &  \\
		& X \ar@{->}[r] \ar@{->}[d] \ar@{->}[lu]
		& X' \ar@{->}[r] \ar@{->}[d] & X''' \ar@{->}[d] \ar@{->}[rd] &  \\
		& Y \ar@{->}[r] \ar@{->}[ld] & Y' \ar@{->}[r]
		& Y''' \ar@{->}[r] & \tilde{Y}''' \\
		Y'' \ar@/_/@{->}[rrru] &  &  &  & 
	}
	\]
	Note that each interior facet commutes
	(all except maybe the right triangle).
	Using this last diagram plus commutativity of
	\eqref{diagram-uniqueness-second-definition-derived-functor},
	one can see that the right triangle commutes
	after precomposition with $X\to X'''$, which is in $S$.
	MS3 implies that there is some $\tilde{Y}'''\to\hat{Y}'''$
	in $S$ such that the triangle commutes after postcomposing with it.
	After replacing $Y'''$ by $\hat{Y}'''$,
	we may assume that we have a diagram
	\[
	\xymatrix{
		F(X'') \ar@{->}[r] \ar@{->}[d] & RF(X) \ar@{->}[r] \ar@{->}[d]^a
		& F(X') \ar@{->}[r] \ar@{->}[d] & F(X''') \ar@{->}[d] \\
		F(Y'') \ar@{->}[r] & RF(Y) \ar@{->}[r] & F(Y') \ar@{->}[r] & F(Y''')
	}
	\]
	where the central, right and outer squares commute.
	It then follows that the left square commutes after
	postcomposition with $RF(Y)\to F(Y')\to F(Y''')$.
	But this last composite is a monomorphism
	(Categories, Remark \ref{categories-remark-essentially-constant-down-the-diagram}),
	and we win.
\end{proof}
...
\medskip\noindent
To see this pick $s : X \to X'$, $s':Y\to Y'$ in $S$,
and morphisms $f':X'\to Y'$,
$\alpha : A \to F(X')$ $\beta:B\to F(Y')$ as in
Lemma \ref{lemma-derived-functor-second-definition}.
Using MS6 choose a morphism of triangles
$$
(s, s', s'') : (X, Y, Z, f, g, h) \longrightarrow (X', Y', Z', f', g', h')
$$
with $s'' \in S$. By TR3 choose a morphism of distinguished triangles
$$
(\alpha, \beta, \gamma) :
(A, B, C, RF(f), b, c)
\longrightarrow
...
We will actually prove that the morphism $\gamma$ is as in
Categories, Definition \ref{categories-definition-essentially-constant-diagram};
from the uniqueness in Lemma \ref{lemma-derived-functor-second-definition}
it will follow that $b=RF(g)$, $c=RF(h)$.
Consider the category $\mathcal{I}$ of
Lemma \ref{lemma-limit-triangles}
of morphisms of distinguished triangles
$$
\mathcal{I} =
...
$$
is bijective.
We finish by Categories,
Lemma \ref{categories-lemma-characterize-essentially-constant-ind}.
The proof for $LF$ is dual.
\end{proof}
...
\medskip\noindent
To show (2), since we have
Lemmas \ref{lemma-derived-shift} and
\ref{lemma-2-out-of-3-defined},
it is left to prove that $RF$ preserves identities and composition.
From the uniqueness in Lemma \ref{lemma-derived-functor-second-definition},
it is immediate that $RF$ preserves identities.
Let $X\xrightarrow{f}Y\xrightarrow{g}Z$ be morphisms
in $\mathcal{D}$.
Lemma \ref{lemma-derived-functor-second-definition}
...
\[
\xymatrix@R=.3pc{
	X \ar@{->}[rr] \ar@{->}[ddd] &  & X' \ar@{->}[dd] \\
	&  &  \\
	&  & Y_1' \\
	Y \ar@{->}[rru] \ar@{->}[rrd] \ar@{->}[ddd] &  &  \\
	&  & Y_2' \ar@{->}[dd] \\
	&  &  \\
	Z \ar@{->}[rr] &  & Z'
}
\]
where all horizontal maps are in $S$, and morphisms $\alpha:RF(X)\to F(X')$,
$\beta_i:RF(Y)\to F(Y_i')$, $i=1,2$, $\gamma:RF(Z)\to F(Z')$
satisfying Categories,
Definition \ref{definition-essentially-constant-diagram},
such that the following diagram commutes:
\[
\xymatrix@R=.3pc{
	RF(X) \ar@{->}[rr]^{\alpha} \ar@{->}[ddd] &  & F(X') \ar@{->}[dd] \\
	&  &  \\
	&  & F(Y_1') \\
	RF(Y) \ar@{->}[rru]^{\beta_1} \ar@{->}[rrd]^{\beta_2} \ar@{->}[ddd] &  &  \\
	&  & F(Y_2') \ar@{->}[dd] \\
	&  &  \\
	RF(Z) \ar@{->}[rr]^{\gamma} &  & F(Z')
}
\]
Let $Y\to Y''$ be an object in $Y/S$ such that there exists a map
$RF(Y)\to F(Y'')$ as in Categories,
Definition \ref{definition-essentially-constant-diagram}.
In particular, $F(Y'')\to RF(Y)$ is epic.
Invoking the property of $\beta_i$ and using that $Y/S$ is filtered,
we get morphisms $Y''\to Y'$, $g_i:Y_i'\to Y'$, $i=1,2$,
all in $Y/S$ such that $F(Y'')\to F(Y')$ equals the composite
$F(Y'')\to RF(Y)\xrightarrow{\beta_i}F(Y_i')\to F(Y')$, for $i=1,2$.
Hence, $\beta:=F(g_1)\circ\beta_1=F(g_2)\circ\beta_2$.
Finally, since $Y_2'\to Y'$ is in $S$ (for $S$ is saturated),
by MS2 we get a commutative diagram
\[
\xymatrix{
	Y_2' \ar@{->}[r] \ar@{->}[d] & Y' \ar@{->}[d] \\
	Z' \ar@{->}[r]^h & \tilde{Z}'
}
\]
with $h\in S$. After substituting $Z'$ by $\tilde{Z}'$
and $\gamma$ by $F(h)\circ\gamma$, we get commutative diagrams
\[
\xymatrix{
	X \ar@{->}[r] \ar@{->}[d]_{f} & X' \ar@{->}[d] &
	& RF(X) \ar@{->}[r]^{\alpha} \ar@{->}[d]_{RF(f)} & F(X') \ar@{->}[d] \\
	Y \ar@{->}[r] \ar@{->}[d]_{g} & Y' \ar@{->}[d] &
	& RF(Y) \ar@{->}[d]_{RF(g)} \ar@{->}[r]^{\beta} & F(Y') \ar@{->}[d] \\
	Z \ar@{->}[r] & Z' &  & RF(Z) \ar@{->}[r]^{\gamma} & F(Z')
}
\]
whence $RF(g\circ f)=RF(g)\circ RF(f)$,
by the uniqueness in Lemma \ref{lemma-derived-functor-second-definition}.
This finishes the proof of (2).

\medskip\noindent
We get (3) from
Lemma \ref{lemma-derived-inverts}. The fully faithfulness in (4) follows
from (3) and Categories,
Lemma \ref{lemma-equality-morphisms-left-localization}. The fact that
...
$\beta  \circ \alpha$ homotopic to zero
(Homology, Lemma \ref{lemma-morphism-kernel-localization}). Then
