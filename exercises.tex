...
\begin{exercise}
	\label{exercise-surjective-induced-morphism-between-kernels}
	...
	$$
	be a cartesian diagram in $\mathcal{A}$. If $P\to A\times_CB$ is a surjection, then the induced morphism from the kernel of $P\to A$ to the kernel of $g$ by the commutative square
	$$
	\xymatrix{
...
Let $K'/K/k$ be field extensions with $K'$
...
is not reduced for some finite field extension $k'/k$.
\end{exercise}
...
and $Y \subset \mathbf{P}^n_k$ is an element $g \in \text{PGL}_n(k)$
such that $g(X) = Y$, and
...
$X$ is irreducible and such that for some finite extension $k'/k$
