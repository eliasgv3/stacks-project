...
$\Ker(\varphi)_x = \Ker(\varphi_x)$. This is because taking stalks commutes with finite limits (as finite limits commute with filtered colimits).
...
\item for a suitable choice of these sections
the kernel of the associated surjection is also
generated by finitely many global sections. (Recall that if $f:x\to y$ is a morphism in an abelian category, then the canonical morphism $x\to\Im f$ is a surjection, see Homology, lemmas \ref{lemma-all-epis-normal-coim-map-monic} and \ref{lemma-characterize-abelian-categories}; and the category of $\mathcal{O}_X$-modules is abelian, Lemma \ref{lemma-abelian}.)
\end{enumerate}
...
Let $(X, \mathcal{O}_X)$ be a ringed space,
and let $\mathcal{L}$ be an $\mathcal{O}_X$-module.
The following are equivalent:
\begin{enumerate}
	\item $\mathcal{L}$ is locally free of rank 1, and
	\item $\mathcal{L}$ is of finite presentation and
	$\mathcal{L}_x\cong\mathcal{O}_{X,x}$ for all $x\in X$.
\end{enumerate}
Moreover, any $\mathcal{O}_X$-module $\mathcal{L}$ satisfying (any of)
the previous equivalent conditions is invertible.
And if all stalks $\mathcal{O}_{X, x}$ are local rings, then
...
\medskip\noindent The implication (1)$\Rightarrow$(2) is clear. The converse follows from Lemma \ref{lemma-finite-presentation-stalk-free}.

\medskip\noindent
Assume now $\mathcal{L}$ is locally free of rank $1$ and consider the
evaluation map
$$
\mathcal{L} \otimes_{\mathcal{O}_X}
\SheafHom_{\mathcal{O}_X}(\mathcal{L}, \mathcal{O}_X)
...
(More on Algebra, Lemma \ref{lemma-local-ring-Pic-trivial}).
On the other hand, we have that $\mathcal{L}$ is of finite presentation by
Lemma \ref{lemma-invertible} (a direct summand of a finite free module is of finite presentation).
\end{proof}
It follows that, over a locally ringed space $(X,\mathcal{O}_X)$, ``being invertible'' is a local property: an $\mathcal{O}_X$-module $\mathcal{L}$ is invertible if and only if there is an open cover $X=\bigcup_{i}U_i$ such that $\mathcal{L}|_{U_i}$ is invertible for all $i$.	
\begin{lemma}
