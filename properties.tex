...
For instance, locally Noetherian schemes have locally finitely
many irreducible components.

\begin{lemma}
\label{lemma-locally-finitely-many-irred-comp-is-local}
Let $X$ be a scheme. Let $X=\bigcup U_i$ be an open cover.
Then $X$ has locally finitely many irreducible components
if and only if $U_i$ has locally finitely many irreducible components,
for all $i$.
\end{lemma}

\begin{proof}
The implication to the left is trivial.
The implication to the right follows from Topology, Lemma
\ref{topology-lemma-correspondence-closed-irreducible-open}.
\end{proof}
