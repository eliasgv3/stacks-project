...
\begin{enumerate}
	\item $X$ is integral,
	\item $X$ is reduced and irreducible, and
	\item for all open $U\subset X$, the ring $\mathcal{O}_X(U)$
	is an integral domain.
\end{enumerate} 
\end{lemma}
...
\medskip\noindent
(2)$\Rightarrow$(3).
Since any open subscheme $U\subset X$ is reduced and irreducible,
it  to show that $R=\Gamma(X,\mathcal{O}_X)$ is an integral domain.
Suppose $f,g\in R$ are such that $fg=0$. Then $X=V(f)\cup V(g)$.
By irreducibility of $X$, we have $X=V(f)$, for example.
We want to show that $f=0$. Since this is a local condition,
we can assume $X$ is affine. But then $f$ belongs to all prime
ideals of $R$ and therefore to its nilradical.
But $R$ is reduced (Lemma \ref{lemma-characterize-reduced}), so $f=0$.
