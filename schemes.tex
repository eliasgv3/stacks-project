...
thanks to Sheaves on Spaces, Lemma \ref{lemma-pullback-commutes-restriction}.
\begin{lemma}
...
\begin{lemma}
	Let $X$ be a locally ringed space. There is a one-to-one correspondence
	\begin{align*}
		\newcommand{\longmapsfrom}{\longleftarrow\!\shortmid}
		\begin{Bmatrix}
			\text{Sheaves of ideals }\mathcal{I}\subset\mathcal{O}_X\\
			\text{locally generated by sections}
		\end{Bmatrix}
		&\longleftrightarrow
		\begin{Bmatrix}
			\text{Isomorphism classes of closed}\\
			\text{immersions }Z\to X\text{ of locally}\\
			\text{ringed spaces over }X
		\end{Bmatrix}\\
		\mathcal{I}
		&\longmapsto
		(\operatorname{Supp}(\mathcal{O}_X/\mathcal{I})\to X)\\
		\Ker i^\sharp
		&\longleftarrow (i:Z\to X)
	\end{align*}
where the morphism
$\operatorname{Supp}(\mathcal{O}_X/\mathcal{I})\to X$
of locally ringed spaces is constructed
in Example \ref{example-closed-subspace}.
\end{lemma}
\begin{proof}
	The composition ``first to the right, then to the left'' equals the identity for what was explained in Example \ref{example-closed-subspace}. The viceversa composition equals the identity by Lemma \ref{lemma-closed-immersion}.
\end{proof}
...
\begin{lemma}
	...
	Let $X=\Spec(R)$ be an affine scheme. The functor
	$$
	(\widetilde{-}):Mod_R\to Mod(\mathcal{O}_X)
	$$
	is faithfully exact (i.e., it preserves and reflects exactness).
	In particular,
...
\begin{proof}
	Let
	\begin{equation}
		\label{equation-SES-tildification-is-exact-1}
		M\to N\to P
	\end{equation}
	be a sequence of $R$-modules, and consider the induced sequence
	\begin{equation}
		\label{equation-SES-tildification-is-exact-2}
		\widetilde{M}\to\widetilde{N}\to\widetilde{P}
	\end{equation}
	of $\mathcal{O}_X$-modules. It is not difficult to see that the induced sequence on stalks at $x\in X$ of \eqref{equation-SES-tildification-is-exact-2} coincides with the localization at $\mathfrak{p}=\mathfrak{p}_x$ of \eqref{equation-SES-tildification-is-exact-1},
	\begin{equation}
		M_{\mathfrak{p}}
		\to N_{\mathfrak{p}}
		\to P_{\mathfrak{p}}.
	\end{equation}
	But exactness of this last sequence for all primes $\mathfrak{p}\subset R$ is equivalent to exactness of \ref{equation-SES-tildification-is-exact-1} (Lemma \ref{lemma-characterize-zero-local}, (3)), and it is also equivalent to exactness of \ref{equation-SES-tildification-is-exact-2} (Lemma \ref{lemma-abelian}).
	
	Exactness of a functor between abelian categories is equivalent to commuting with kernels and cokernels. Therefore, for a map $\varphi:M\to N$ of $R$-modules, we have
	$\Ker(\widetilde{\varphi}) = \widetilde{\Ker(\varphi)}$ and
	$\Coker(\widetilde{\varphi}) = \widetilde{\Coker(\varphi)}$.
\end{proof}
