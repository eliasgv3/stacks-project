...
is a sequence $\emptyset\neq Z_0 \subset Z_1 \subset \ldots \subset Z_n \subset X$
...
\begin{lemma}
	\label{lemma-krull-dim-sup-cover}
	Let $X$ be a topological space and let $X=\bigcup U_i$ be an open cover. Then $\dim(X)=\sup_i\dim(U_i)$.
\end{lemma}
\begin{proof}
	($\geq$) It suffices to prove that if $Y\subset X$ is any subspace then $\dim(Y)\leq\dim(X)$. But this follows from the fact that the closure of an irreducible subspace is irreducible: if $\emptyset\neq Z_0\subset\cdots\subset Z_n$ is a chain of irreducible closed subsets of $Y$, then $\overline{Z}_0\subset\cdots\subset\overline{Z}_n$ is a chain of irreducible closed subsets of $X$. The inclusions in the latter are indeed strict since intersecting with $Y$ brings us back to the original chain.
	
	\medskip\noindent
	($\leq$) Let $\emptyset\neq Z_0\subset\cdots\subset Z_n$ be a chain of irreducible closed subsets of $X$. Let $U$ be one of the $U_i$ such that there is $z_0\in Z_0\cap U_i$. We want to see that $Z_k\cap U$ is a chain of irreducible subsets of $U$. On the one hand, these sets are irreducible since an open subset of an irreducible space needs to be irreducible. On the other hand, the inclusion $Z_k\cap U\subset Z_{k+1}\cap U$ must be strict: Suppose that instead it were an equality. Then $Z_k$ and $Z_{k+1}\setminus U$ covers $Z_{k+1}$, so one of equals $Z_{k+1}$, and that has to be $Z_k$ since $z_0\notin Z_{k+1}\setminus U$.
\end{proof}
